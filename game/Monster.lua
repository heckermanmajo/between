...
function Monster.new(x, y, level)
    ...
    if x == nil then
        ...
        local monster_tile = level:get_random_floor_tile()
        while monster_tile.kind == 0 do monster_tile = level:get_random_floor_tile() end
...
function Monster:collide_with_walls(dt)
    local monster_tile = Level.current_level:get_tile_at(self.x, self.y)
    if monster_tile.kind == 0 then
        pushCircleOut(self, { x = monster_tile.x + Level.current_level.TILE_SIZE / 2, y = monster_tile.y + Level.current_level.TILE_SIZE / 2, radius = Level.current_level.TILE_SIZE / 2 }, dt)
    end
end
...
function Monster:wander(dt)
    ...
    if distance_to_player < 100 then
        ...
        if m.target_tile == nil then
            local tiles = Level.current_level:get_tiles_around(m.x, m.y)
        ...
        if m.target_tile == Level.current_level:get_tile_at(m.x, m.y) then
            m.target_tile = nil
        end

        if m.target_tile then
            local dx = (m.target_tile.x + Level.current_level.TILE_SIZE / 2) - m.x
            local dy = (m.target_tile.y + Level.current_level.TILE_SIZE / 2) - m.y
            m.rotation = math.atan2(dy, dx)
            local magnitude = math.sqrt(dx ^ 2 + dy ^ 2)
            if magnitude > 0 then
                dx = dx / magnitude
...
function Monster:target_player()
    ...
    if Level.current_level:get_tile_at(m.x, m.y).visible then
        -- add some small advantage to the player: monster dont see as far as the player
        local bresenham_line_to_player = bresenham_line(Level.current_level:get_tile_at(m.x, m.y), Level.current_level:get_tile_at(Player.x, Player.y))
        local distance_in_tiles = #bresenham_line_to_player
        if distance_in_tiles < 5 then
            m.target_tile = Level.current_level:get_tile_at(Player.x, Player.y)
        end
    end
end
...
function Monster.point_hit_monster(x, y)
    ...
    for _, m in ipairs(Level.current_level.monsters) do
...
function Monster:delete()
    -- todo: LEVEL is a very bad global variable here ...
    for i, m in ipairs(Level.current_level.monsters) do
        if m == self then
            table.remove(Level.current_level.monsters, i)
            return
        end
    end
end
